...
class FootballGameProcessor:
    ...
    def run(self):
        ...
        with self.video_sink:
            ...
            for frame in tqdm(self.frame_generator, total=self.video_info.total_frames):
                ...
                if player_with_ball != None and self.last_player_with_ball != None and player_with_ball != self.last_player_with_ball:
                    print(player_with_ball, self.last_player_with_ball)
                    print(all_detections[all_detections.tracker_id == player_with_ball].class_id, all_detections[all_detections.tracker_id == self.last_player_with_ball].class_id)
                ...
                labels = [f"#{tracker_id} - {self.player_velocities[tracker_id][-1]:.2f} km/h" for tracker_id in all_detections.tracker_id]
        ...
        arquivo_saida = "src/test/metricas_finais.txt"
        ...
        SoccerVisualizer.plot_gaussian_heatmap(self.player_positions)
        ...
        with open(arquivo_saida, "w") as arquivo:
            ...
            for team, metric in metrics.items():
                ...
                for metric_name, value in metric.items():
                    # Verifica se o valor é uma lista (como a centróide) e formata adequadamente
...
if __name__ == "__main__":
    source_video_path = "src/test/test.mp4"
    target_video_path = "src/test/football_output.mp4"
