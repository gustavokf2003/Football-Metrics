...
class SoccerVisualizer:
    ...
    def plot_gaussian_heatmap(player_positions, field_size=(3000, 4600), sigma=1):
        """
        Cria um mapa de calor suavizado por Gauss para cada jogador com base em suas posições.

        Args:
            player_positions (dict): Dicionário onde a chave é o id do jogador e o valor é uma lista de arrays
                                    representando as posições do jogador no campo [[x1, y1], [x2, y2], ...].
            field_size (tuple): Dimensões do campo (largura, altura), padrão é 3000x4500.
            grid_size (int): Tamanho do grid para a resolução do histograma.
            sigma (float): Parâmetro para suavização Gaussiana.
        ...
        for player_id, positions in player_positions.items():
            ...
            filepath = os.path.join('src/test', f"heatmap_player_{player_id}.png")
